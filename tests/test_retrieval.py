...
def test_retriever_initialization_and_chunking(temp_docs_dir):
    """Test that the retriever loads and chunks documents correctly."""
    retriever = TfidfRetriever(docs_path=str(temp_docs_dir))
    ...
    # Check if the doc_ids are correct
    expected_ids = [
...
def test_retriever_search_top_k(temp_docs_dir):
    """Test that the search function returns the correct number of results."""
    retriever = TfidfRetriever(docs_path=str(temp_docs_dir))
    
    # Search for "yellow" and expect 1 relevant chunk, even with k=2
    results = retriever.search("yellow", top_k=2)
    assert len(results) == 1
    
    # The first result should be the chunk containing "yellow"
    assert results[0]["id"] == "doc2::chunk1"
    assert "yellow" in results[0]["content"]

def test_retriever_search_content_and_score(temp_docs_dir):
...
def test_retriever_search_no_match(temp_docs_dir):
    """Test a query that should not match any document well."""
    retriever = TfidfRetriever(docs_path=str(temp_docs_dir))
    
    results = retriever.search("unrelated topic like elephants")
    # A robust test can check that the highest score is below a threshold
    if results:
        assert results[0]["score"] < 0.5 # Arbitrary low threshold for a non-matching query
        
def test_retriever_empty_docs_directory(tmp_path):
    """Test that the retriever handles an empty docs directory gracefully."""
    empty_docs_dir = tmp_path / "empty_docs"
    empty_docs_dir.mkdir()
    
    retriever = TfidfRetriever(docs_path=str(empty_docs_dir))
